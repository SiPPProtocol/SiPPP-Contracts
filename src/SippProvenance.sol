...
contract SippProvenance is AccessControl {
    ...
    function registerUser(Privy _privy, uint256 _farcasterId, address _primaryAccount, string _name, string _email)
        public
        onlyApp
    {
        require(
            registeredUsers[_privy].primaryAccount == 0x0000000000000000000000000000000000000000,
    ...
    function registerDevice(Privy _privy, UserDevice memory _device) public onlyApp {
        require(
            registeredUsers[_privy].primaryAccount != 0x0000000000000000000000000000000000000000, "User not registered"
        );
    ...
    function registerPhoto(Privy _privy, uint256 deviceId, PhotoProvenance memory _photo) public {
        require(
            registeredUsers[_privy].devices[deviceId].user != 0x0000000000000000000000000000000000000000,
            "Device not found"
        );
    ...
    function confirmPhotoPhrase(Privy _privy, UserDevice memory _device, PhotoProvenance memory _algoProvenance)
        public
        onlyApp
    {
        require(
            registeredUsers[_privy].photos[_algoProvenance.photoHash].photoHash == _algoProvenance.photoHash,
            "Photo not found"
        );
